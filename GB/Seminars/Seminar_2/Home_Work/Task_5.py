...
def mix_list(int_list):  # Функция перемешивания списка
    if len(int_list) < 2:
        ...
        new_mix_list = ['+' for i in range(
            len(int_list))]  # Создаем новый список заполненный '+', которые будут затираться по мере заполнения
        for i in range(len(int_list)):
            flag = True
            while flag:
                num_rand = random.randint(0,
                                          len(int_list) - 1)  # Задаем рандомное число от 0 до длины списка перемешивания-1
                if new_mix_list[
                    num_rand] == '+' and num_rand != i:  # Если в новом списке под индексом ранд. числа '+' и оно не равно i,
